...
/**
 * @brief Execute the KICK command (kick a user from a channel)
 * @details The user must be an operator of the channel to kick another user. The kicked user will receive a KICK message
 * and will be removed from the channel. If the command is called without arguments, if the target user is not in the
 * channel or if the channel doesn't exist, an error message will be sent to the user.
 *
 * @param message The KICK message to process.
 */
void CmdKick::execute(Message *message) {
    ...
    for (size_t i = 0; i < targets.size(); i++) {
        ...
        if (user == target) {
            reply(serv_name,
                  ERR_UNKNOWNERROR(user->getNickname(), "KICK", "you cannot kick yourself. Use /PART instead."), fd);
            return;
        }
