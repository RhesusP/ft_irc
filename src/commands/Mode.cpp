...
/**
 * @brief Handle the mode string and apply the changes to the channel.
 * @details The mode string is parsed and the modes are added or removed from the channel. The mode arguments are also
 * parsed and used to set the mode parameters.
 * @param args The arguments of the MODE command.
...
void CmdMode::handleModeString(std::vector<std::string> args, Channel *chan, User *user) {
    ...
    for (size_t i = 0; i < modeString.size(); i++) {
        ...
        add ? chan->addMode(modeString[i], j < mode_args.size() ? mode_args[j] : "", user) : chan->removeMode(
                modeString[i], j < mode_args.size() ? mode_args[j] : "", user);
        j++;
    }
}

/**
 * @brief Execute the MODE command (change user or channel modes)
 * @details If the target is a user, the command will return the user's mode. If the target is a channel, the command
 * will return the channel's mode. If the user is not an operator of the channel, an error message will be sent.
 * @param message The MODE message to process.
 */
    ...
    if (args[0][0] != '#' && args[0][0] != '&') {
        ...
        if (args.size() == 1) {
            ...
            this->reply(serv_name,
                        RPL_CREATIONTIME(user->getNickname(), target->getName(), timestr(target->getCreationTime())),
                        fd);
            return;
        }
